...
The Accept header is constructed by the scrape target to indicate what formats
it supports.
...
Prometheus SHOULD use the following process to select an appropriate
...
1. It MUST use the protocol in the Accept header with the highest weighting that
   is supported by the scraper.
2. If no protocols are supported, the scraper MAY use a user-configured fallback
   scrape protocol.
3. If no fallback is specified, scraper MUST use PrometheusText0.0.4 as a
   last resort.

## Content-Type Response

Scrapers SHOULD respond with a Content-Type header that matches one of the
...
1. Scrapers MUST validate the Accept header to prevent potential injection attacks
