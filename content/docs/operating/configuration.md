...
* **node**: Discovers one target per cluster node with the address defaulting
  ...
  The target address defaults to the first existing address of the Kuberentes
  ...
  * `__meta_kubernetes_node_address_<address_type>`: The first address for each node address type, if exists.

  In addition, the `instance` label for node will be set to the node name
...
* **endpoints**: Discovers targets from listed endpoints of a service. For each endpoint
  one target is discovered per target endpoint. If the endpoint is backed by a pod, all
  additional container ports of the pod, not bound to an endpoint port, are discoverd as targets as well.
