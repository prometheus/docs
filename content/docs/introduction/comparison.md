...
Sensu is a primarily agent-based monitoring solution ([reference](https://docs.sensu.io/sensu-go/latest/operations/deploy-sensu/deployment-architecture/)). Sensu agents automatically connect to one or more Sensu backends over an encrypted pub/sub transport (TLS-encrypted websockets), and handle automated registration/de-registration of entities and local service discovery. Sensu [backends](https://docs.sensu.io/sensu-go/latest/observability-pipeline/observe-schedule/backend/) provide the control plane, including the configuration [API](https://docs.sensu.io/sensu-go/latest/api/) and [scheduler](https://docs.sensu.io/sensu-go/latest/observability-pipeline/observe-schedule/). 
