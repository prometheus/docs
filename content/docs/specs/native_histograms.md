...
MAY contain a mix of samples of different types. Changes of the sample type
within a time series are expected to be very rare in practice. They usually
happen after changes in the instrumentation of a target (in the rare case that
the same metric name is used for e.g. a gauge float prior to the change and a
counter histogram after the change) or after a change of a recording rule (e.g.
where the old version of a rule created a gauge float and the new version of
rule now creates a gauge histogram while retaining its name). Frequent changes
of the sample type are usually the consequence of a misconfiguration (e.g. two
different recording rules creating different sample types feeding into the same
series). Therefore, a TSDB implementation MUST handle a change in sample type,
but it MAY do so in a relatively inefficient way. When the Prometheus TSDB
encounters a sample type that cannot be written to the currently used chunk, it
closes that chunk and starts a new one with the appropriate encoding. (A time
series that switches sample types back and forth for each sample will lead to a
new chunk for each sample, which is indeed very inefficient.)
...
- Only for schema -53 (NHCB) the custom values, encoded as the number of custom
  valuse (varbit-uint), followed by the custom values in a repeated sequence,
  using a custom encoding.

The chunk layout is followed by a repeated sequence of sample data. The sample
date is different for integer histograms and float histograms. For an integer
histogram, the data for each sample contains the following:
...
suitable for efficient storage (as the histogram version is essentially just a
storage optimization). This works for both float and integer histograms (as the
sum field is a float value even in an integer histogram), and the appropriate
version can be used to avoid cutting a new chunk. All version of a stale marker
(float, integer histogram, float histogram) MUST be treated as equivalent when
querying the TSDB.

### Chunk size limit

The size of float chunks is limited to 1024 bytes. The same size limitation is
generally used for histogram chunks, too. However, individual histograms could
...
a single histogram could take more than 1024 bytes so that the size limit
cannot be enforced at all.) With very few histograms per chunk, the compression
ratio becomes worse. Therefore, a minimum number of 10 histograms per chunks
has to be reached before the size limit of 1024 bytes kicks in. This implies
that histogram chunks can be much larger than 1024 bytes.

The current simplistic approach might be improved in the future to find a
better trade-off between chunk size and compression ratio.

### Counter reset considerations

Generally, Prometheus considers a counter to have reset whenever its value
drops (but see also the [next section about the created
timestamp](#created-timestamp-handling)). The situation is more complex with
histograms. Any of the following constitutes a counter reset:
...
- Any change of schema _unless_ the change happens from one standard
  exponential schema to another towards a lower resolution (i.e. decreasing
  schema number). The latter happens regularly as a consequence of reducing the
  resolution, while all other schema changes can only happen after a histogram
  has been reset or newly created. See also special case below.
- A change of the custom values (relevant for NHCBs.
- A decrease of the zero bucket threshold, which can only happen after a
  histogram has been reset or newly created.
- An increase of the zero bucket threshold where the new zero bucket partially
  overlaps with a populated regular bucket in the previous histogram, which
  again can only happen after a histogram has been reset or newly created.

// describe the special case: lower resolution, compatible groth of zero
// bucket, still need to detect in the modified histogram

// because it is expensive: counter reset hint, only for the 1st sample
...
// always float
// merging (adjust schema, NHCB compatibility)
...
described in this section. This is not called out separately further down. See
[tracking issue](https://github.com/prometheus/prometheus/issues/13934) for
details.

### Binary operators

// discuss scaled and "negative" histograms

### Aggregation operators

### Functions

// rate and friends: warn if gauge (and warn for delta if counter (is
// Prometheus doing that already?)

// histogram_quantile: how to calculate max/min, usage difference to classic, examples

// histogram specific functions: why needed compared to classic, examples,
// optimization for count/sum.

### Recording rules

// May store result as int histogram.
