...
for repo in $REPOS; do
    ...
    git clone https://github.com/prometheus/$repo

    cd $repo

    git checkout release-${LATEST}
    cp -rf docs $ROOT/content/docs/$repo/latest

    for version in $VERSIONS; do
        echo "Copying docs for ${repo} version $version"
        git checkout release-$version
        git status
        cp -rf docs $ROOT/content/docs/prometheus/$version
