...
Prometheus side (hint: prefer the latter), while OTel exponential histograms
...
`rate(http_request_duration_seconds_sum[5m])` above, throwing off the
calculated value. Note that this problem only affects summaries and classic
...
underlying metric type is. These are the expressions to calculate the fleet-wide
average request duration by `handler`:

Native histograms:

      sum by (handler) (histogram_sum(rate(http_request_duration_seconds[5m])))
    /
      sum by (handler) (histogram_count(rate(http_request_duration_seconds[5m])))
...
because each NHCB is represented by a single series of complex samples (rather
than by a separate series of floats for each bucket and the sum and count of
observations).
...
cause higher resource usage, but generally you can reach much higher resolution
...
between 0ms and 300ms (or 0.3s) in the last 5m. Note how the `sum` correctly
aggregates by summing up the corresponding buckets in the involved histograms.
If the histograms have different bucket layouts, they are reconciled first.
With the usual exponential bucketing schema, this works smoothly, essentially
by falling back to the lowest common resolution among all involved histograms.
The same is done to reconcile different bucket layouts over time (in the 5m
range that is used in the `rate` calculation). With NHCBs, the effects of this
depend heavily on the details of the different bucket layouts. It is well
possible that the reconciled aggregated histogram has just one bucket left,
containing all observations. Because of the potentially severe effects, the
query result gets an info-level annotation if NHCBs needed to be reconciled.
This is also one of the reasons why native histograms with the dynamic
exponential buckets are much easier to handle.
...
above.) If you pick native histograms with the dynamic exponential buckets and
a decent resolution, you get reasonably accurate estimations, including the
...
failed requests from the satisfied and tolerable parts of the calculation, as
...
| Query performance | The server has to calculate quantiles from complex histogram samples. You can use [recording rules](/docs/prometheus/latest/configuration/recording_rules/#recording-rules) should the ad-hoc calculation take too long (e.g. in a large dashboard). | The server has to calculate quantiles from a large number of bucket series. You can use [recording rules](/docs/prometheus/latest/configuration/recording_rules/#recording-rules) should the ad-hoc calculation take too long (e.g. in a large dashboard). | Fost (no quantile calculations on the server, and aggregations are impossible, see below).
...
estimate the 95th percentile to be 0.228 or 228ms (again glossing over the
details of the calculation here). However, the true value could be anywhere
between 210ms and 229ms, depending on the actual distribution of requests
within the bucket. However, this is a fairly accurate estimation.

Now let's apply the same to the classic histogram configued as described above.
All observations, and therefore also the 95th percentile, will fall into the
bucket labeled `{le="0.3"}`, i.e. the bucket from 200ms to 300ms. The
...
The corresponding bucket of the native histogram ranged from 297ms to 324ms,
with the interpolated estimation for the 95th percentile being 323ms. That's an
...
the classic histogram or a general relative bucket width in case of a native
histogram). With a broad distribution, small changes in φ result in large
deviations in the observed value. With a sharp distribution, a small interval
of observed values covers a large interval of φ.
...
The rules of thumb are the following:
  ...
  3. Only if aggregation isn't needed, you can start thinking about summaries.
     ...
     (at least in the dimension of φ) at relatively low cost (which is paid by
     the instrumented program, not by the Prometheus server). However, the
     additional requirement to pick the desired quantiles and sliding window at
     instrumentation time is another big drawback of summaries.
