...
Prometheus project occasionally participate in various, periodic mentoring programs:
...
> NOTE: For Prometheus to participate in a mentorship cycle, potential mentors with at 
> the least one Prometheus maintainer have to propose a "project" to a program cycle, e.g. [like in this PR](https://github.com/cncf/mentoring/pull/1490).
>
...
- [X] Send your mentors your Slack account handle, so we can add you to private channels. We tend to have a private one with just you and your main mentors, and second with all mentees, ex-mentees and mentors (`#thanos-prometheus-mentees`).
- [X] Join main Prometheus Slack channels [1](#slack-channel), [2](#slack-channel-1) for future interactions with the community.
- [X] On the [dev Prometheus Slack channel](#slack-channel-1), write a few sentences to about yourself, say hello! 💜
...
- [X] Read our [contributing details](#contributing), also one on [Prometheus repo](https://github.com/prometheus/prometheus/blob/main/CONTRIBUTING.md)
...
On the mid-point and a last meeting of the mentorship it's best to sit down with a mentee and collect *actionable learnings* for both mentors and a mentee. The experience of both Mentors and Mentees is always different due to differences among us, different task and circumstances. We always want to check on our experience, no matter if we met the initial goals or not. If goals were not met, then it's extra important to discuss this in honest, blameless atmosphere. If the experience was mostly positive we also want to know what worked to reinforce best practices.

The example retrospective process looks as follows:

* Week before the meeting remind Mentees about the planned retrospective and how it will look like (e.g. link this guide). This allows everyone to think through the week about things that worked and not.
* During the meeting, start first with the retrospective. This is the most important part of meeting, don't let other things (e.g. project status) to get in the path.
    * Write in the working doc two sections: `What we did well`, `What we could do better`.
    ...
    * Consider composing all learnings into some for or [a public content](#public-speaking-or-writing).
...
* Encourage mentee to write a blog post on [Prometheus](https://github.com/prometheus/docs/tree/main/blog-posts) or personal blog!
